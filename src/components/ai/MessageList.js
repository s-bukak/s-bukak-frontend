...
const MessageList = ({ isOpenModal = false }) => {
  ...
  const submitComment = async () => {
    ...
    try {
      const response = await fetch('https://017a-203-246-112-24.ngrok-free.app/generate', {
        method: 'POST',
                ...
                >
                  {comment.userId === 'user' ? (
                    <div className="flex flex-col items-end max-w-xs mb-1">
                      <div className="bg-gray-200 text-gray-700 p-2.5 rounded-2xl text-sm break-words">
                        {comment.content}
                      </div>
                      <span className="text-xs w-full text-gray-500 mt-1">{comment.createdAt}</span>
                    </div>
                  ) : (
                    <div className="flex items-start max-w-xs w-full relative mb-4">
                      ...
                      <div className="flex-col relative z-0 left-3.5">
                        <div className="bg-gray-700 text-white p-3 rounded-2xl text-sm break-words">
                          {comment.content}
                        </div>
